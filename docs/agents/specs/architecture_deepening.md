...
2. As a framework consumer, I want `serverkit.NewDefaultHandler` to return a `chi.Router`, so that I don't have to unsafely type-assert the return value to mount my application routes.
...
- **Router Seam**: Updated `serverkit.NewDefaultHandler` to return `chi.Router` and exposed `WithRouterTimeout` as a functional option.
